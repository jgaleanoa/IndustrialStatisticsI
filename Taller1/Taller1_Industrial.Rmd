...
media aumenta para diferentes valores de $n.$

```{r oc-xbar}
oc.curves(control_xbar1, n = 1:4*5)
```
...
valores de $\beta$ son muy cercanos a cero para $n \in \{5,10,15\}$, sin
...
mu1 <- x_doble_bar + k*sigma1
for(i in 1:4*5){
  beta <- pnorm(16.577256, mu1, sigma1/sqrt(i)) - 
    pnorm(15.900744, mu1, sigma1/sqrt(i))
  n <- rep(i, length(beta))
  if(i == 5) {
    aux_mat <- cbind(mu1, beta, n)
  }
  aux_mat <- rbind(aux_mat, cbind(mu1, beta, n)) 
}
aux_mat <- aux_mat[-(1:200), ] %>% 
  cbind(k = rep(k, 4)) %>% 
  as.data.frame() %>% 
  mutate(n = as.factor(n))

ggplot(aux_mat, aes(k, beta)) +
